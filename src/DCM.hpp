...
public:
    ...
    DCM(std::initializer_list<std::initializer_list<T>> list);
...
//! Construct using initializer list
template<class T>
DCM<T>::DCM(std::initializer_list<std::initializer_list<T>> list):
    SquareMatrix<T, 3>(list)
{
}
...
DCM<T>::DCM(const Euler<T> &e)
{
    ...
    switch(e.getRotatationSequence())
    {
        ...
        {
            this->data[0] = -std::sin(a1)*std::cos(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
            this->data[1] = -std::sin(a1)*std::cos(a2)*std::cos(a3) - std::cos(a1)*std::sin(a3);
        ...
        case RotationSequence::XYX_121:
        {
            ...
            this->data[2] = std::sin(a2)*std::cos(a3);
            this->data[3] = std::sin(a1)*std::sin(a2);
            this->data[4] = std::cos(a1)*std::cos(a3) - std::sin(a1)*std::cos(a2)*std::sin(a3);
            this->data[5] = -std::cos(a1)*std::sin(a3) - std::sin(a1)*std::cos(a2)*std::cos(a3);
            this->data[6] = -std::cos(a1)*std::sin(a2);
            this->data[7] = std::sin(a1)*std::cos(a3) + std::cos(a1)*std::cos(a2)*std::sin(a3);
            this->data[8] = -std::sin(a1)*std::sin(a3) + std::cos(a1)*std::cos(a2)*std::cos(a3);
        } break;

        case RotationSequence::YZY_232:
        {
            this->data[0] = std::cos(a1)*std::cos(a2)*std::cos(a3) - std::sin(a1)*std::sin(a3);
            this->data[1] = -std::cos(a1)*std::sin(a2);
            this->data[2] = std::cos(a1)*std::cos(a2)*std::sin(a3) + std::sin(a1)*std::cos(a3);
            this->data[3] = std::sin(a2)*std::cos(a3);
            this->data[4] = std::cos(a2);
            this->data[5] = std::sin(a2)*std::sin(a3);
            this->data[6] = -std::sin(a1)*std::cos(a2)*std::cos(a3) - std::cos(a1)*std::sin(a3);
            this->data[7] = std::sin(a1)*std::sin(a2);
            this->data[8] = -std::sin(a1)*std::cos(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
        } break;

        case RotationSequence::ZYZ_323:
        {
            this->data[0] = std::cos(a1)*std::cos(a2)*std::cos(a3) - std::sin(a1)*std::sin(a3);
            this->data[1] = -std::cos(a1)*std::cos(a2)*std::sin(a3) - std::sin(a1)*std::cos(a3);
            this->data[2] = std::cos(a1)*std::sin(a2);
            this->data[3] = std::sin(a1)*std::cos(a2)*std::cos(a3) + std::cos(a1)*std::sin(a3);
            this->data[4] = -std::sin(a1)*std::cos(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
            this->data[5] = std::sin(a1)*std::sin(a2);
            this->data[6] = -std::sin(a2)*std::cos(a3);
        ...
        {
            this->data[0] = std::cos(a2);
            this->data[1] = -std::sin(a2)*std::cos(a3);
            this->data[2] = std::sin(a2)*std::sin(a3);
            this->data[3] = std::cos(a1)*std::sin(a2);
            this->data[4] = std::cos(a1)*std::cos(a2)*std::cos(a3) - std::sin(a1)*std::sin(a3);
            this->data[5] = -std::cos(a1)*std::cos(a2)*std::sin(a3) - std::sin(a1)*std::cos(a3);
            this->data[6] = std::sin(a1)*std::sin(a2);
            this->data[7] = std::sin(a1)*std::cos(a2)*std::cos(a3) + std::cos(a1)*std::sin(a3);
            this->data[8] = -std::sin(a1)*std::cos(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
        } break;
        ...
        case RotationSequence::YXY_212:
        {
            ...
            this->data[2] = std::sin(a1)*std::cos(a2)*std::cos(a3) + std::cos(a1)*std::sin(a3);
            this->data[3] = std::sin(a2)*std::sin(a3);
            this->data[4] = std::cos(a2);
            this->data[5] = -std::sin(a2)*std::cos(a3);
            this->data[6] = -std::cos(a1)*std::cos(a2)*std::sin(a3) - std::sin(a1)*std::cos(a3);
        ...
        case RotationSequence::XYZ_123:
        {
            ...
            this->data[3] = std::sin(a1)*std::sin(a2)*std::cos(a3) + std::cos(a1)*std::sin(a3);
            this->data[4] = -std::sin(a1)*std::sin(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
            this->data[5] = -std::sin(a1)*std::cos(a2);
            this->data[6] = -std::cos(a1)*std::sin(a2)*std::cos(a3) + std::sin(a1)*std::sin(a3);
            this->data[7] = std::cos(a1)*std::sin(a2)*std::sin(a3) + std::sin(a1)*std::cos(a3);
        ...
        case RotationSequence::YZX_231:
        {
            ...
            this->data[1] = -std::cos(a1)*std::sin(a2)*std::cos(a3) + std::sin(a1)*std::sin(a3);
            this->data[2] = std::cos(a1)*std::sin(a2)*std::sin(a3) + std::sin(a1)*std::cos(a3);
            this->data[3] = std::sin(a2);
            this->data[4] = std::cos(a2)*std::cos(a3);
            this->data[5] = -std::cos(a2)*std::sin(a3);
            this->data[6] = -std::sin(a1)*std::cos(a2);
            this->data[7] = std::sin(a1)*std::sin(a2)*std::cos(a3) + std::cos(a1)*std::sin(a3);
            this->data[8] = -std::sin(a1)*std::sin(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3) ;
        } break;

        case RotationSequence::ZXY_312:
        {
            this->data[0] = -std::sin(a1)*std::sin(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
            this->data[1] = -std::sin(a1)*std::cos(a2);
            this->data[2] = std::sin(a1)*std::sin(a2)*std::cos(a3) + std::cos(a1)*std::sin(a3);
            this->data[3] = std::cos(a1)*std::sin(a2)*std::sin(a3) + std::sin(a1)*std::cos(a3);
            this->data[4] = std::cos(a1)*std::cos(a2);
            this->data[5] = -std::cos(a1)*std::sin(a2)*std::cos(a3) + std::sin(a1)*std::sin(a3);
        ...
        case RotationSequence::XZY_132:
        {
            ...
            this->data[3] = std::cos(a1)*std::sin(a2)*std::cos(a3) + std::sin(a1)*std::sin(a3);
            this->data[4] = std::cos(a1)*std::cos(a2);
            this->data[5] = std::cos(a1)*std::sin(a2)*std::sin(a3) - std::sin(a1)*std::cos(a3);
            this->data[6] = std::sin(a1)*std::sin(a2)*std::cos(a3) - std::cos(a1)*std::sin(a3);
            this->data[7] = std::sin(a1)*std::cos(a2);
            this->data[8] = std::sin(a1)*std::sin(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
        } break;
        ...
        {
            this->data[0] = std::cos(a1)*std::cos(a2);
            this->data[1] = std::cos(a1)*std::sin(a2)*std::sin(a3) - std::sin(a1)*std::cos(a3);
            this->data[2] = std::cos(a1)*std::sin(a2)*std::cos(a3) + std::sin(a1)*std::sin(a3);
            this->data[3] = std::sin(a1)*std::cos(a2);
            this->data[4] = std::sin(a1)*std::sin(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
            this->data[5] = std::sin(a1)*std::sin(a2)*std::cos(a3) - std::cos(a1)*std::sin(a3);
        ...
        {
            this->data[0] = std::sin(a1)*std::sin(a2)*std::sin(a3) + std::cos(a1)*std::cos(a3);
            this->data[1] = std::sin(a1)*std::sin(a2)*std::cos(a3) - std::cos(a1)*std::sin(a3);
            this->data[2] = std::sin(a1)*std::cos(a2);
            this->data[3] = std::cos(a2)*std::sin(a3);
            this->data[4] = std::cos(a2)*std::cos(a3);
            this->data[5] = -std::sin(a2);
            this->data[6] = std::cos(a1)*std::sin(a2)*std::sin(a3) - std::sin(a1)*std::cos(a3);
            this->data[7] = std::cos(a1)*std::sin(a2)*std::cos(a3) + std::sin(a1)*std::sin(a3);
            this->data[8] = std::cos(a1)*std::cos(a2);
        } break;
